...
class GamesCornerScreen extends StatefulWidget {
  const GamesCornerScreen({super.key});

  @override
  State<GamesCornerScreen> createState() => _GamesCornerScreenState();
}

class _GamesCornerScreenState extends State<GamesCornerScreen> {
  ...
  bool _onlyBoules = false;
  bool _onlyTableTennis = false;
  ...
  Future<void> _loadData() async {
    try {
      ...
      final all = await OverpassService.fetchMultipleFields(
        ...
        sportTypes: ["boules", "table_tennis"],
      );
  ...
  void _applyFilters() {
    _filteredFields = _allFields.where((field) {
      ...
      if (_onlyBoules && sport != 'boules') return false;
      if (_onlyTableTennis && sport != 'table_tennis') return false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(_onlyBoules ? Icons.sports : Icons.sports_outlined),
            tooltip: "Toggle Boules",
            onPressed: () {
              setState(() {
                _onlyBoules = !_onlyBoules;
                _applyFilters();
              });
            },
          ),
          IconButton(
            icon: Icon(_onlyTableTennis ? Icons.sports_tennis : Icons.sports_tennis_outlined),
            tooltip: "Toggle Table Tennis",
            onPressed: () {
              setState(() {
                _onlyTableTennis = !_onlyTableTennis;
                _applyFilters();
              });
            },
          ),
